...
nums = [[8, 2, 22, 97, 38, 15, 00, 40, 00, 75,
...
def solve():
    ...
    for i in range(len(nums)):
        for j in range(len(nums[i])):
            answer = max(answer, reduce(lambda x, y: x * y, nums[i][j: j + 4]))
            answer = max(answer, reduce(lambda x, y: x * y,
                                        [m[j] for m in nums[i: i + 4]]))
            if i < 17 and j < 17:
                answer = max(answer, nums[i][j] * nums[i + 1][j + 1]
